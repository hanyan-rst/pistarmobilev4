...
if ( $testMMDVModeDMR == 1 ) {
  ...
  if ( !empty($_POST) && ( isset($_POST["dropDyn"]) || isset($_POST["dropQso"]) || isset($_POST["refSubmit"]) || isset($_POST["tgSubmit"]) ) ): // Data has been posted for this page
    ...
    // Clean up...
    unset($_POST);
    echo '<script type="text/javascript">setTimeout(function() { window.location.replace("../index.php");},5000);</script>';
...
<style>
 ...
 select  {
	...
	font-size: 13px;
     ...
     <b>BrandMeister Manager</b>
     <table width="140" border="0">
  <tr>
    <th colspan="3"><a class=tooltip href="#">Tools<span><b>DMR ID</b></span></a></th>
    </tr>
  <tr>
    <td colspan="3">
    <input type="submit" value="Drop QSO" name="dropQso" />
    <input type="submit" value="Drop All Dynamic" name="dropDyn" />
    </td>
    </tr>
  <tr>
  ...
  <tr>
    <td>
      <select name="reflectorNr">
      <?php if ( $bmReflectorActive == "None" || $bmReflectorActive == "REF0" ) { 
	  echo '        <option selected="selected" value="0">None</option>'."\n"; } 
	  else { 
	  echo '        <option value="0">None</option>'."\n"; } 
        for ($refNrBase = 1; $refNrBase <= 999; $refNrBase++) {
          $refNr = 4000 + $refNrBase;
          if ( "REF".$refNr == $bmReflectorActive ) { 
		  echo '        <option selected="selected" value="'.$refNr.'">REF'.$refNr.'</option>'."\n"; }
          else { 
		  echo '        <option value="'.$refNr.'">REF'.$refNr.'</option>'."\n"; }
        } ?>
      </td>
      <td>
      <input type="radio" name="REFmgr" value="LINK" />Link <input type="radio" name="REFmgr" value="UNLINK" checked="checked" />UnLink</td>
      <td>
      <input type="submit" value="Modify Reflector" name="refSubmit" /></td>
  </tr>
  <tr>
    <th colspan="3"><a class=tooltip href="#">Static Talkgroup<span><b>Enter the Talkgroup number</b></span></a></th>

  </tr>
   <tr>
	<td colspan="3"><center><input type="text" name="tgNr" size="10" maxlength="7" /></center></td>
  </tr>
   <tr>
   ...
   <tr>
    <td><input type="radio" name="TS" value="1" />TS1 <input type="radio" name="TS" value="2" checked="checked" />TS2</td>
      <td><input type="radio" name="TGmgr" value="ADD" checked="checked" />Add <input type="radio" name="TGmgr" value="DEL" />Delete</td>
      <td><input type="submit" value="Modify Static" name="tgSubmit" /></td>
  </tr>
</table>
<br />
